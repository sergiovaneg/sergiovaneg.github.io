...
The first step is to parse the input. As a sequence of input/output pairs with distinct input subgrids, it follows that the natural structure to store the rule set is a hash-map. Since in Go, slices (i.e., arrays of unknown size) are not hashable, we use the flattened pattern strings as keys and the expanded subgrids as values. This results in the following type definitions and function signatures (trivial implementation):
